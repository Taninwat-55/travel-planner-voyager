            ...
            <div className="space-y-4 max-h-80 overflow-y-auto pr-2">
              {hotels.length > 0 ? (
                hotels.map((hotel, idx) => (
                  <div key={idx} className="flex justify-between items-center bg-gray-100 rounded-xl p-4 shadow-md">
                    <div>
                      <h4 className="font-bold text-lg">{hotel.name}</h4>
                      <p className="text-sm text-gray-600">
                        ⭐ {hotel.rating} • {hotel.reviews} reviews • {hotel.distance} miles from city center
                      </p>
